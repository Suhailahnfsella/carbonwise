  ...
  <div>
    <div class="fixed top-4 left-4 z-50 lg:hidden flex flex-col items-start space-y-2">
      <button @click="toggleSidebar" class="p-2 bg-white rounded shadow" aria-label="Toggle sidebar">
        ...
        <svg v-else xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-darkblue" fill="none" viewBox="0 0 24 24"
          stroke="currentColor">
    ...
    <aside :class="[
      ...
      <nav class="space-y-2 mt-6">
        <RouterLink v-for="item in navItems" :key="item.name" :to="item.path"
          ...
          :class="{ 'bg-greenlight/20 font-semibold': isActive(item.path) }" @click="sidebarOpen = false">
          {{ item.name }}
        </RouterLink>
      </nav>
    </aside>

    <div v-if="sidebarOpen" @click="sidebarOpen = false" class="fixed inset-0 bg-black/30 z-30 lg:hidden"></div>
